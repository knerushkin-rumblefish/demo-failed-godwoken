...
export async function swapWithRegistry(admin: string, deployer: Wallet, transactionOverrides: Overrides) {
  ...
  if(swaps && registry) {
    ...
    const exchangeAmount = await swaps.get_exchange_amount(
      poolAddress,
      tokensAddresses[0],
      tokensAddresses[1],
      tokensAmounts[0],
    ...
    console.log('exchange amount', exchangeAmount.toString())
...
export async function updateRegistrySwaps() {
